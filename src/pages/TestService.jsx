...
// ── Image Imports ──
import imgHeroBg    from "../assets/logo/powerstation3.jpg";   // substation — hero bg
import imgStation   from "../assets/logo/station.jpg";         // motor control centre — services
import imgDataCenter from "../assets/logo/datacenter.png";     // data centre aisle — specialized
import imgFuelStation from "../assets/logo/fuelstation.jpg";   // fuel station — hazardous/fuel
import imgSolar from "../assets/logo/solar1.jpg";           // solar panels — solar/renewables
import imgInstallation from "../assets/logo/solar2.jpg"; // solar installation — installation/supervision
...
/* ── Small reusable: service scope list item ── */
...
/* ── Image banner section used for each service ── */
function ServiceImageBanner({ img, alt, label, accent = false }) {
  return (
    <div className="relative h-56 overflow-hidden">
      ...
      />
      <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(13,33,55,0.85) 0%, transparent 60%)" }} />
      {accent && <div className="absolute top-0 left-0 w-[3px] h-full bg-[#8B1A1A]" />}
      <div className="absolute bottom-4 left-5">
        <span className="font-sans text-[10px] tracking-[0.22em] uppercase text-[#F59E0B]">{label}</span>
      </div>
    </div>
  );
}
...
export default function Services() {
  ...
  // Map service index to image
  const serviceImages = [imgStation, imgDataCenter, imgFuelStation];
  const serviceImageLabels = ["LV / MV Design Work", "Site Supervision", "Specialized Installations"];
  ...
  return (
    <main className="bg-[#F8F4F0] overflow-x-hidden">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: "88vh" }}>
        ...
        <div
          ...
          style={{
            ...
            filter: "brightness(0.42) grayscale(10%)",
        ...
        <div
          ...
          style={{ background: "linear-gradient(105deg, rgba(13,33,55,0.97) 0%, rgba(13,33,55,0.80) 45%, rgba(139,26,26,0.28) 100%)" }}
        />
        ...
        <div
          ...
          style={{ minHeight: "88vh", paddingTop: "150px", paddingBottom: "72px" }}
        >
          <div className="flex items-center gap-3 mb-8" style={{ animation: "fadeDown 0.7s ease both" }}>
            <span className="w-8 h-[2px] bg-[#8B1A1A]" />
            <span className="font-sans text-[11px] tracking-[0.28em] uppercase text-[#F59E0B]">
              What We Offer
            </span>
          </div>

          <h1
            className="font-heading font-bold text-white leading-[0.93] mb-8"
            style={{ fontSize: "clamp(50px, 9vw, 108px)", animation: "fadeUp 0.8s ease 0.1s both" }}
          >
            Our<br />
            <span className="text-[#8B1A1A]">Services</span><br />
            &amp;<br />
            <span className="text-white/85">Expertise</span>
          </h1>

          <p
            className="font-body text-white/75 text-lg max-w-xl leading-relaxed mb-12"
            style={{ animation: "fadeUp 0.8s ease 0.25s both", borderLeft: "2px solid rgba(139,26,26,0.5)", paddingLeft: "18px" }}
          >
          ...
          <div className="flex flex-wrap gap-4" style={{ animation: "fadeUp 0.8s ease 0.4s both" }}>
            <Link
              to="/contact"
              className="font-sans text-[11px] tracking-widest uppercase bg-[#8B1A1A] text-white px-8 py-4 hover:bg-[#6e1515] transition-all duration-300 active:scale-95"
              onMouseEnter={(e) => (e.currentTarget.style.boxShadow = "0 8px 28px rgba(139,26,26,0.45)")}
            ...
            <a
              href="#services"
              className="font-sans text-[11px] tracking-widest uppercase border border-white/40 text-white px-8 py-4 transition-all duration-300 hover:border-[#F59E0B] hover:text-[#F59E0B]"
            >
              Explore Services
            </a>
          </div>
      ...
      {/* ══════════════════════════════════════════
          CORE SERVICES — TABBED WITH IMAGE PANELS
      ══════════════════════════════════════════ */}
      <section id="services" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Tab bar */}
          <div className="flex flex-col sm:flex-row gap-0 border-b border-[#0D2137]/10 mb-0">
            {coreServices.map(({ id, title }, i) => (
              <button
                key={id}
                onClick={() => { setActiveService(i); setExpandedService(null); }}
          ...
          {coreServices.map(({ id, title, icon, shortDesc, fullDesc, items }, i) => (
            <div key={id} style={{ display: activeService === i ? "block" : "none" }}>
              <div className="grid lg:grid-cols-5 gap-0 border border-[#0D2137]/08 mt-0">
                ...
                <div className="lg:col-span-2 relative overflow-hidden group" style={{ minHeight: "420px" }}>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="font-sans text-[10px] tracking-[0.2em] uppercase text-[#F59E0B]">
                        Service {id}
                      </span>
                    </div>
                ...
                <div className="lg:col-span-3 p-8 lg:p-12 bg-[#F8F4F0]">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 bg-[#8B1A1A] flex items-center justify-center text-white flex-shrink-0">
                      {icon}
                    </div>
                    <p className="font-body text-[#0D2137]/70 text-sm leading-relaxed max-w-md">
                      {shortDesc}
                    </p>
                  </div>
                  ...
                  {expandedService === i ? (
                    <>
                      ...
                      <button
                        ...
                        className="font-sans text-[11px] tracking-widest uppercase text-[#8B1A1A] flex items-center gap-2 hover:gap-4 transition-all duration-300"
                      >
                    ...
                    <button
                      ...
                      className="font-sans text-[11px] tracking-widest uppercase bg-[#8B1A1A] text-white px-6 py-3 hover:bg-[#6e1515] transition-all duration-300 mt-2"
                    >
                      Read More
                    </button>
                  )}
      ...
      {/* ══════════════════════════════════════════
          IMAGE SHOWCASE STRIP — 3 project photos
      ══════════════════════════════════════════ */}
            ...
            {training.map(({ title, desc, icon }, i) => (
              <FadeIn key={title} delay={i * 100}>
                <div className="bg-white border border-[#0D2137]/10 p-8 group hover:border-[#8B1A1A] transition-all duration-500 h-full relative overflow-hidden">
                  ...
                  <div className="w-11 h-11 bg-[#0D2137] group-hover:bg-[#8B1A1A] flex items-center justify-center text-white mb-6 transition-colors duration-300">
                    {icon}
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
            {projects.map((project, i) => (
              <FadeIn key={project.num} delay={Math.min(i * 55, 380)}>
                <div className="group border border-white/10 hover:border-[#8B1A1A]/60 transition-all duration-300 h-full flex flex-col relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-0 h-[2px] bg-[#8B1A1A] group-hover:w-full transition-all duration-500" />

                  {/* Card header */}
                  <div className="flex items-start justify-between p-6 pb-3">
                    <span className="font-heading text-4xl font-bold text-white/10 leading-none">{project.num}</span>
                    <span className={`font-sans text-[10px] font-bold tracking-wider uppercase px-2 py-1 border ${tagColors[project.tag] || "bg-white/10 text-white/50"}`}>
                      {project.tag}
                    </span>
                  </div>
                  ...
                  <div className="px-6 pb-5 flex-1 flex flex-col">
                    <h3 className="font-heading text-lg font-bold text-white mb-1 leading-snug">{project.title}</h3>
                    ...
                    {expandedProject === i ? (
                      <>
                        <p className="font-body text-white/60 text-sm leading-relaxed flex-1 mb-4">{project.desc}</p>
                        <button
                          onClick={() => setExpandedProject(null)}
                          className="font-sans text-[10px] tracking-widest uppercase text-[#8B1A1A] self-start hover:text-[#F59E0B] transition-colors"
                        >
                          Show Less ↑
                        </button>
                      </>
                      ...
                      <button
                        ...
                        className="font-sans text-[10px] tracking-widest uppercase text-white/30 hover:text-[#F59E0B] self-start transition-colors mt-auto"
                      >
                        Read More ↓
                      </button>
                    )}
      ...
      <section className="relative bg-[#8B1A1A] py-20 overflow-hidden text-center">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <FadeIn>
            ...
            <Link
              to="/contact"
              className="inline-block font-sans text-sm tracking-widest uppercase bg-white text-[#8B1A1A] px-10 py-5 hover:bg-[#F8F4F0] transition-all duration-300 active:scale-95 font-bold"
            >
              Get in Touch
            </Link>
          </FadeIn>
