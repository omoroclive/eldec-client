...
const projects = [
  { num: "01", title: "EV Charging Stations", location: "Kenya & Rwanda", desc: "Design and installation of EV charging infrastructure with cumulative capacity of 7,500 kW including transformers, LV distribution and DC fast chargers.", tag: "Energy" },
  { num: "02", title: "Kooba Data Center", location: "Mombasa, Kenya", desc: "Tier 3 data center design — USD 2.5M project including RMU systems, transformers, UPS, generators and dual power supply configuration.", tag: "Data Center" },
  { num: "03", title: "BATUK Phase 2A", location: "Laikipia, Kenya", desc: "KES 442M electrical design and supervision for accommodation, catering and support facilities including street lighting and fire alarm systems.", tag: "Infrastructure" },
  { num: "04", title: "Off-Grid Solar Installation", location: "Remote Facility", desc: "80 kWp solar PV system including inverters, protection systems, charge controllers and full commissioning in a remote off-grid facility.", tag: "Solar" },
];
    ...
    <main className="bg-[#F8F4F0] overflow-x-hidden">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: "95vh" }}>
        ...
        <div
          ...
          style={{
            background: "linear-gradient(105deg, rgba(13,33,55,0.98) 0%, rgba(13,33,55,0.85) 40%, rgba(139,26,26,0.3) 100%)",
          }}
        />
        ...
        {/* Main Hero Content - Increased top padding to ensure text clarity */}
        <div
          className="relative z-10 max-w-7xl mx-auto px-6 flex flex-col justify-center"
          style={{ 
            minHeight: "95vh", 
            paddingTop: "160px", // Increased from 120px to push content down
            paddingBottom: "80px" 
          }}
        >
          {/* Eyebrow - Enhanced readability with shadow and slightly larger font */}
          <div className="flex items-center gap-4 mb-8" style={{ animation: "fadeDown 0.7s ease both" }}>
            <span className="w-10 h-[2px] bg-[#8B1A1A]" />
            <span className="font-sans text-[12px] md:text-[14px] tracking-[0.3em] uppercase text-[#F59E0B] font-semibold" style={{ textShadow: "0 2px 4px rgba(0,0,0,0.5)" }}>
          ...
          <p
            className="font-body text-white/80 text-lg md:text-xl max-w-2xl leading-relaxed mb-12"
            style={{ animation: "fadeUp 0.8s ease 0.25s both", fontWeight: 300 }}
          >
            ELDEC Limited delivers electrical engineering consultancy, design, installation and project supervision 
            across Africa — built to international standards, every time.
          </p>

          <div className="flex flex-wrap gap-5" style={{ animation: "fadeUp 0.8s ease 0.4s both" }}>
            <Link to="/contact" className="font-sans text-[12px] tracking-widest uppercase bg-[#8B1A1A] text-white px-10 py-5 hover:bg-[#a61f1f] transition-all duration-300 active:scale-95 shadow-lg">
              Start a Project
            </Link>
            <Link to="/services" className="font-sans text-[12px] tracking-widest uppercase border border-white/60 text-white px-10 py-5 transition-all duration-300 hover:border-[#F59E0B] hover:text-[#F59E0B] backdrop-blur-sm">
              Our Services
            </Link>
          </div>
        </div>
      ...
      {/* ══════════════════════════════════════════
          STATS BAR
      ══════════════════════════════════════════ */}
      <section className="relative z-10" style={{ background: "rgba(13,33,55,1)", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <FadeIn>
              ...
              <p className="font-body text-[#0D2137] leading-relaxed mb-4 text-justify">
                ELDEC Limited is a private limited company incorporated in Kenya in 2025 to provide professional Electrical Engineering Services, 
                Consultancy, Design, and Project Supervision.
              </p>
              <Link to="/about" className="inline-flex items-center gap-3 font-sans text-sm tracking-widest uppercase text-[#8B1A1A] group">
            ...
            <FadeIn delay={150}>
              <div className="relative">
                ...
                <div className="overflow-hidden">
                  <img src={imgPanel} alt="ELDEC LV Panel" className="w-full h-[480px] object-cover object-center hover:scale-105 transition-transform duration-700" />
                  <div className="absolute bottom-0 left-0 right-0 bg-[#0D2137]/80 backdrop-blur-sm px-6 py-4">
                    <p className="font-sans text-xs tracking-[0.2em] uppercase text-[#F59E0B]">Real Project Work</p>
                    <p className="font-body text-white text-sm mt-1">LV Distribution Board — Commissioned & Certified</p>
                  </div>
                </div>
              </div>
            </FadeIn>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <FadeIn className="text-center mb-16">
            ...
            <h2 className="font-heading text-4xl lg:text-5xl font-bold text-[#0D2137]">Our Core Services</h2>
            ...
            {services.map(({ icon, title, desc, items }, i) => (
              <FadeIn key={title} delay={i * 120}>
                <div className="group border border-[#0D2137]/10 p-8 hover:border-[#8B1A1A] transition-all duration-500 hover:shadow-xl hover:shadow-[#8B1A1A]/10 relative overflow-hidden bg-[#F8F4F0]">
                  ...
                  <div className="mb-6 transform group-hover:-translate-y-1 transition-transform duration-500">{icon}</div>
      ...
      <section className="py-24 bg-[#0D2137] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <FadeIn className="mb-16">
            ...
            <h2 className="font-heading text-4xl lg:text-5xl font-bold text-white">Featured Projects</h2>
              ...
              <FadeIn key={title} delay={i * 100}>
                <div className="group border border-white/10 p-8 hover:border-[#8B1A1A]/60 transition-all duration-500 bg-white/5">
                  <div className="flex items-start justify-between mb-6">
                    <span className="font-heading text-5xl font-bold text-white/10">{num}</span>
                    <span className="font-sans font-bold text-xs text-[#F59E0B] border border-[#F59E0B]/30 px-3 py-1">{tag}</span>
                  </div>
                  <h3 className="font-heading text-xl font-bold text-white mb-1">{title}</h3>
                  <p className="font-sans text-xs text-[#8B1A1A] mb-4 uppercase">{location}</p>
              ...
              <FadeIn key={title} delay={i * 80}>
                <div className="flex gap-5 group">
                  <div className="flex-shrink-0 w-12 h-12 bg-white border border-[#8B1A1A]/20 rounded-xl flex items-center justify-center group-hover:bg-[#8B1A1A] transition-all">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-6 h-6 text-[#8B1A1A] group-hover:text-white">
      ...
      <section className="relative bg-[#8B1A1A] py-24 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-white">
          <FadeIn className="max-w-2xl">
            <h2 className="font-heading text-4xl lg:text-5xl font-bold mb-6">Let's Start Your Next Project Together</h2>
            <p className="font-body text-lg mb-10 opacity-80">From design to commissioning — ELDEC delivers engineering solutions built to last.</p>
            <div className="flex flex-wrap gap-4">
              <Link to="/contact" className="bg-white text-[#8B1A1A] px-10 py-4 uppercase text-sm tracking-widest font-sans">Get in Touch</Link>
            </div>
          </FadeIn>
        </div>
      </section>
